...
class Database {
    ...
    static async signinUser(userinfo, onComplete) {
        const users = await this._getUsers();
        const user = this._findUser(users, userinfo.username);
        if (user) {
            if (userinfo.username == user.username) {
                await AsyncStorage.setItem("signedInUser", JSON.stringify(user));
                onComplete("ok", user);
            }else{
                onComplete("Wrong Password!", user);
            }
        }else{
            onComplete("User doesn't exist!", user);
        }
    }
